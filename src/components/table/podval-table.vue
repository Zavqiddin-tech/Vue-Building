    ...
    <Table v-if="Array.isArray(podval) && podval.length > 0">
      ...
      <TableBody>
        <TableRow v-for="item in podval" class="hover:bg-black/10">
          ...
          <TableCell>
            <span v-if="convertDate(item.selectDate, 1)" class="py-1 px-3   rounded-full bg-lime-300 shadow-lg">
              {{convertDate(item.selectDate, 1)}}
            </span>
            <span v-else class="py-1 px-3 text-white   rounded-full bg-red-400 shadow-lg">
              {{ item.selectDate }}
            </span>
            </TableCell
          >
